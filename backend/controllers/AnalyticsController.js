      ...
      try {
        ...
        const geoResponse = await axios.get(
          `https://nominatim.openstreetmap.org/search`,
          {
            ...
            headers: {
              "User-Agent": "CECC-Clinic-App/1.0 (contact@cecc.com)",
            },
          }
        );
        ...
        if (geoResponse.data && geoResponse.data.length > 0) {
          ...
          console.log(`Geocode not found for: ${query}`);
        }
      } catch (geoError) {
        console.error(
          `Geocoding failed for: ${cacheKey}`,
          geoError.response?.status,
          geoError.response?.statusText
        );
      }
    }
