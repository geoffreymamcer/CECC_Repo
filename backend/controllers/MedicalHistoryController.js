...
export const getMedicalHistoryByPatientId = async (req, res) => {
  try {
    ...
    if (!medicalHistory) {
      console.log(
        `No medical history found with patientId field, trying alternative lookups`
      );
      ...
      if (profile) {
        console.log(
          `Found profile with ID: ${profile._id}, looking for medical history with this ID`
        );
        // Try to find medical history with the profile's ID
        medicalHistory = await MedicalHistory.findOne({
          $or: [{ patientId: profile._id }, { patientId: profile.patientId }],
        });
      }
    }
...
export const deleteMedicalHistory = async (req, res) => {
  try {
    const medicalHistory = await MedicalHistory.findByIdAndDelete(
      req.params.id
    );
